...
export default function AuditorLaporan() {
  const { user, activeOutletId } = useAuthStore()
  const [activeTab, setActiveTab] = useState('keuangan') // 'keuangan' | 'rekap'
  const [data, setData] = useState({ revenue: [], byProduct: [], aktivitasPerJam: [] })
  ...
  const fetchLaporan = async () => {
    ...
    try {
      ...
      const res = await laporanService.getKeuangan({
        ...dates,
      ...
      setData({ revenue: [], byProduct: [], aktivitasPerJam: [] })
    }
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-zinc-200 pb-4">
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-0.5">Pantau laporan keuangan dan rekap harian (Read-Only)</p>
          </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-zinc-100">
                        {['Produk', 'Total Terjual', 'Omzet'].map(h => (
                    ...
                    <tbody>
                      {loading ? (
                        [...Array(4)].map((_, i) => (
                          <tr key={i}><td colSpan={3} className="px-5 py-3"><div className="h-8 bg-zinc-100 rounded-lg animate-pulse" /></td></tr>
                        ))
                      ) : data.byProduct.length === 0 ? (
                        <tr><td colSpan={3} className="px-5 py-12 text-center text-zinc-400 text-sm">Belum ada data produk</td></tr>
        ...
        {activeTab === 'rekap' && <TabRekapKasir />}
      </div>
    </Layout>
  )
}

function TabRekapKasir() {
                ...
                <tr key={r.id} className="hover:bg-zinc-50/80 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-2">
                      ...
                      <button 
                        onClick={() => navigate(`/auditor/transaksi?date=${r.tanggal}`)}
                        className="text-xs font-bold text-zinc-600 hover:text-zinc-900 bg-zinc-100 hover:bg-zinc-200 px-3 py-1.5 rounded-xl transition-colors w-full text-left flex items-center gap-1.5 mt-1"
                      >
